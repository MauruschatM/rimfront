...
export function BuildingsRenderer({ buildings, stats, }) {
    return (<group>
      {buildings.map((b) => {
            ...
            {isUnderConstruction && (<mesh position={[centerX, centerY, 1.5]}>
            ...
            {!isUnderConstruction && (<>
                ...
                {/* Icon text */}
                <Text anchorX="center" anchorY="middle" fontSize={1.5} position={[centerX, centerY, 2.6]}>
                  {icon}
                </Text>

                {/* Active/Max count (top-right of icon) */}
                {capacity > 0 && (<Text anchorX="left" anchorY="bottom" color={stat.active > 0 ? "#4ade80" : "#ef4444"} fontSize={0.8} position={[centerX + 1.3, centerY + 0.8, 2.7]}>
                    {stat.active}/{capacity}
                  </Text>)}

                {/* Spawn timer (bottom-right of icon) */}
                {showSpawnTimer && stat.total < capacity && (<Text anchorX="left" anchorY="top" color="#94a3b8" fontSize={0.6} position={[centerX + 1.3, centerY - 0.8, 2.7]}>
                    {timeToSpawn}s
                  </Text>)}

                {/* Working/Sleeping count (bottom-left of icon) */}
                {(stat.working > 0 || stat.sleeping > 0) && (<Text anchorX="right" anchorY="top" color={stat.working > 0 ? "#fbbf24" : "#60a5fa"} fontSize={0.6} position={[centerX - 1.3, centerY - 0.8, 2.7]}>
                    {stat.working > 0
                            ? `⚙${stat.working}`
                            : `💤${stat.sleeping}`}
                  </Text>)}
            ...
            {b.captureStart && (<group position={[centerX, b.y + b.height + 1.5, 3]}>
                <mesh position={[0, 0, 0]}>
