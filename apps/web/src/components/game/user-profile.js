...
export function UserProfile({ user }) {
    ...
    return (<div className="pixel-border relative z-10 mx-auto flex max-w-md flex-col items-center space-y-4 border-4 border-primary bg-card/80 p-8 backdrop-blur-sm">
      ...
      <div className="w-full">
        <Dialog onOpenChange={setOpen} open={open}>
          <DialogTrigger asChild>
            <Button className="w-full border-primary/50 border-dashed font-mono text-xs hover:border-primary" variant="outline">
              ...
              <TabsContent className="space-y-4 pt-4" value="profile">
                <Form {...form}>
                  <form className="space-y-4" onSubmit={form.handleSubmit(onUpdateProfile)}>
                    ...
                    <Button className="w-full rounded-none font-sans" disabled={isLoading} type="submit">
              ...
              <TabsContent className="space-y-6 pt-4" value="system">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button className="h-8 font-mono text-xs" onClick={() => setTheme("light")} size="sm" variant={theme === "light" ? "default" : "outline"}>
                      LIGHT
                    </Button>
                    <Button className="h-8 font-mono text-xs" onClick={() => setTheme("dark")} size="sm" variant={theme === "dark" ? "default" : "outline"}>
                      DARK
                    </Button>
                  </div>
                </div>
                ...
                <div className="border-border border-t pt-4">
                  <Button className="w-full rounded-none font-sans" disabled={isLoggingOut} onClick={onLogout} variant="destructive">
