...
export function GameCanvas({ game, staticMap, buildings, players, entities, isBuildMode, selectedBuilding, onPlaceBuilding, selectedTroopId, onSelectTroop, onMoveTroop, families, troops, myPlayerId, }) {
    const placeBase = useMutation(api.game.placeBase);
    const isDraggingRef = useRef(false);
    const [flyTo, setFlyTo] = useState(null);
    const lastZoomedPhaseRef = useRef(null);
    const [diplomacyTargetId, setDiplomacyTargetId] = useState(null);
    // Handle Zoom Trigger
    useEffect(() => {
    ...
    const isConfused = myPlayer?.lastBetrayalTime &&
        Date.now() < myPlayer.lastBetrayalTime + 60_000;
    if (!staticMap)
        return null;
    const energyTiles = useMemo(() => {
        if (!(isBuildMode && myPlayerId))
            return new Set();
    ...
    const { visibleEntities, visibleBuildings, buildingStats, entityMap, attackingUnits, memberEntities, commanderEntities, soldierEntities, turretGunEntities, workingEntityPositions, } = useMemo(() => {
        // 1. Identify Allies
        const alliedPlayerIds = new Set();
        if (myPlayerId) {
            ...
            if (alliances) {
                for (const a of alliances) {
                    if (a.status === "allied") {
                        if (a.player1Id === myPlayerId)
                            alliedPlayerIds.add(a.player2Id);
                        if (a.player2Id === myPlayerId)
                            alliedPlayerIds.add(a.player1Id);
                    }
                }
            }
        }
        ...
        const isVisible = (x, y, ownerId, type) => {
            if (alliedPlayerIds.has(ownerId))
                return true;
            if (type === "base_central")
                return true; // Always see Enemy Central Base
            const cx = Math.floor(x / CHUNK_SIZE);
            const cy = Math.floor(y / CHUNK_SIZE);
            return visibleChunks.has(`${cx},${cy}`);
        };
        const filteredBuildings = buildings.filter((b) => isVisible(b.x + b.width / 2, b.y + b.height / 2, b.ownerId, b.type));
        ...
        for (const entity of filteredEntities) {
            ...
            if (!entity.isInside) {
                ...
                else if (entity.type === "turret_gun")
                    turretGuns.push(entity);
            }
    ...
    return (<React.Fragment>
      <Canvas camera={{
        ...
        }} className="cursor-crosshair" gl={{ antialias: false }} orthographic>
        <color args={["#000"]} attach="background"/>
        <ambientLight intensity={0.5}/>
        <directionalLight intensity={1} position={[10, 10, 10]}/>

        <CameraManager flyTo={flyTo} isDraggingRef={isDraggingRef} mapHeight={staticMap.height} mapWidth={staticMap.width}/>
        ...
        {entities && (<LasersRenderer attackingUnits={attackingUnits} entityMap={entityMap}/>)}

        {entities && (<UnitsRenderer commanders={commanderEntities} entities={visibleEntities} families={memberEntities} isDraggingRef={isDraggingRef} onSelectTroop={onSelectTroop} selectedTroopId={selectedTroopId} soldiers={soldierEntities} turretGuns={turretGunEntities}/>)}

        {entities && (<IncomeIndicator workingEntities={workingEntityPositions}/>)}

        <InteractionPlane buildings={visibleBuildings} energyTiles={energyTiles} game={game} height={staticMap.height} isBuildMode={isBuildMode} isDraggingRef={isDraggingRef} onClick={handlePlace} onMoveTroop={onMoveTroop} selectedBuilding={selectedBuilding} selectedTroopId={selectedTroopId} staticMap={staticMap} width={staticMap.width}/>
      </Canvas>
      {diplomacyTargetId && myPlayerId && (<DiplomacyModal gameId={game._id} isOpen={!!diplomacyTargetId} myPlayerId={myPlayerId} onClose={() => setDiplomacyTargetId(null)} targetPlayerId={diplomacyTargetId}/>)}
      {isConfused && (<div className="pointer-events-none fixed inset-0 z-50 animate-pulse bg-red-500/10 mix-blend-multiply"/>)}
    </React.Fragment>);
}
