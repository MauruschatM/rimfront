...
export class SpatialMap {
     ...
     */
    findNearbyEnemies(x: number, y: number, radius: number, myOwnerId: string): { units: Unit[], buildings: Building[] } {
        const enemies: Unit[] = [];
        ...
        for (let cx = centerCx - radiusChunks; cx <= centerCx + radiusChunks; cx++) {
            for (let cy = centerCy - radiusChunks; cy <= centerCy + radiusChunks; cy++) {
                const key = `${cx},${cy}`;
    ...
    findClosestTarget(x: number, y: number, radius: number, myOwnerId: string): { type: "unit" | "building", target: Unit | Building, dist: number } | null {
        const { units, buildings } = this.findNearbyEnemies(x, y, radius, myOwnerId);

        let bestTarget: { type: "unit" | "building", target: Unit | Building, dist: number } | null = null;
