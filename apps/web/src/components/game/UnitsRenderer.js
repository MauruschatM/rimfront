...
export function LasersRenderer({ entityMap, attackingUnits, }) {
    const lasers = useMemo(() => {
        return attackingUnits
            ...
            return {
                ...
                end: [target.x + jitterX, target.y + jitterY, 1],
            };
        })
            .filter((l) => !!l);
    }, [attackingUnits, entityMap]);
    return (<group>
      {lasers.map((l) => (<Line color="blue" key={l.id} lineWidth={2} opacity={0.8} points={[l.start, l.end]} transparent/>))}
    </group>);
}
export function UnitsRenderer({ entities, families, commanders, soldiers, turretGuns, selectedTroopId, onSelectTroop, isDraggingRef, }) {
    const familiesRef = useRef(null);
    const commandersRef = useRef(null);
    const soldiersRef = useRef(null);
    const turretGunsRef = useRef(null);
    const interpolation = useInterpolatedUnits(entities);
    useFrame((state) => {
        const updateMesh = (mesh, list) => {
            if (!mesh)
                return;
            const tempObj = new THREE.Object3D();
            list.forEach((entity, i) => {
                const pos = entity.type === "turret_gun"
                    ? { x: entity.x, y: entity.y }
                    : interpolation.getInterpolatedPosition(entity._id, entity.x, entity.y);
                ...
                if (entity.type === "turret_gun" && entity.attackTargetId) {
                    ...
                    if (target) {
                        const tPos = interpolation.getInterpolatedPosition(target._id, target.x, target.y);
                ...
                const scale = entity.type === "commander"
                    ? 0.8
                    : entity.type === "soldier"
                        ? 0.4
                        : entity.type === "turret_gun"
    ...
    const handleUnitClick = useCallback((e, list) => {
    ...
    return (<group>
      {families.length > 0 && (<instancedMesh args={[undefined, undefined, families.length]} ref={familiesRef}>
          <sphereGeometry args={[0.4, 8, 8]}/>
          <meshBasicMaterial />
        </instancedMesh>)}
      {commanders.length > 0 && (<instancedMesh args={[undefined, undefined, commanders.length]} onClick={(e) => {
      ...
      {soldiers.length > 0 && (<instancedMesh args={[undefined, undefined, soldiers.length]} onClick={(e) => handleUnitClick(e, soldiers)} ref={soldiersRef}>
          <boxGeometry args={[0.8, 0.8, 1]}/>
          <meshStandardMaterial />
        </instancedMesh>)}
      {turretGuns.length > 0 && (<instancedMesh args={[undefined, undefined, turretGuns.length]} ref={turretGunsRef}>
