...
export const requestAlliance = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existing = await ctx.db
            .query("diplomacy")
            .withIndex("by_gameId", (q) => q.eq("gameId", args.gameId))
            .filter((q) => q.or(q.and(q.eq(q.field("player1Id"), player._id), q.eq(q.field("player2Id"), args.targetPlayerId)), q.and(q.eq(q.field("player1Id"), args.targetPlayerId), q.eq(q.field("player2Id"), player._id))))
            .first();
        const now = Date.now();
        if (existing) {
            ...
            if (player.lastBetrayalTime && now < player.lastBetrayalTime + 60_000) {
                throw new Error("You recently betrayed an alliance. Trust must be earned.");
            }
        }
...
export const acceptAlliance = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const player = await ctx.db
            .query("players")
            .filter((q) => q.and(q.eq(q.field("gameId"), diplomacy.gameId), q.eq(q.field("userId"), identity.subject)))
            .first();
        if (!player)
            throw new Error("Player not found");
...
export const renewAlliance = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (diplomacy.status !== "allied")
            throw new Error("Not allied");
        const player = await ctx.db
            .query("players")
            .filter((q) => q.and(q.eq(q.field("gameId"), diplomacy.gameId), q.eq(q.field("userId"), identity.subject)))
            .first();
        if (!player)
            throw new Error("Player not found");
        if (player._id !== diplomacy.player1Id &&
