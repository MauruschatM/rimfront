...
export function handleWalking(member, now, workshops, houses) {
...
export function handleWorking(member, now, mapWidth, mapHeight, blocked, houses) {
...
export function handleSleeping(member, now) {
...
export function handleIdleLogic(member, now, mapWidth, mapHeight, blocked, target, workshops, allEntities, isRoundTick, isConfused) {
    ...
    if (effectiveTarget &&
        ...
        if (!member.nextPathAttempt || now >= member.nextPathAttempt) {
            const path = findPath({ x: member.x, y: member.y }, effectiveTarget, mapWidth, mapHeight, blocked);
            if (path) {
                member.path = path;
                member.pathIndex = 0;
                member.state = "moving";
    ...
    if ((member.state === "idle" || member.state === "patrol") &&
        ...
        if (member.type === "member" &&
            ...
            if (member.reservedFactoryId && !isRoundTick) {
                const reservedWorkshop = workshops.find((w) => w.id === member.reservedFactoryId);
                if (reservedWorkshop && reservedWorkshop.ownerId === member.ownerId) {
                    const targetX = reservedWorkshop.x + Math.floor(reservedWorkshop.width / 2);
                    const targetY = reservedWorkshop.y - 1;
                    const path = findPath({ x: member.x, y: member.y }, { x: Math.max(0, targetX), y: Math.max(0, targetY) }, mapWidth, mapHeight, blocked);
                    if (path && path.length > 0) {
                        member.path = path;
                        member.pathIndex = 0;
                        member.state = "moving";
            ...
            if (!member.reservedFactoryId || isRoundTick) {
                let bestWorkshop = null;
                let bestDist = Number.POSITIVE_INFINITY;
                for (const w of workshops) {
                    if ((w.constructionEnd && now < w.constructionEnd) ||
                    ...
                    const dist = Math.sqrt((w.x + w.width / 2 - member.x) ** 2 +
        ...
        if (Math.random() < 0.4) {
            ...
            const tx = Math.max(0, Math.min(mapWidth - 1, anchorX +
                Math.floor(Math.random() * (patrolRadius * 2 + 1)) -
                patrolRadius));
            const ty = Math.max(0, Math.min(mapHeight - 1, anchorY +
                Math.floor(Math.random() * (patrolRadius * 2 + 1)) -
                patrolRadius));
...
export function updateMember(member, now, mapWidth, mapHeight, blocked, target, workshops, houses, allEntities, isRoundTick, isConfused) {
    ...
    if (handleIdleLogic(member, now, mapWidth, mapHeight, blocked, target, workshops, allEntities, isRoundTick, isConfused)) {
        return true;
    }
    return false;
}
export function categorizeBuildings(buildings) {
    const houses = [];
    const workshops = [];
    const barracks = [];
    const turrets = [];
