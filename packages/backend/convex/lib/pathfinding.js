 ...
 */
export function createCollisionMap(width, height, buildings, tiles) {
 ...
 */
function findNearestWalkable(target, width, height, blocked) {
 ...
 */
export function findPath(start, end, width, height, blocked) {
    ...
    while (openList.size() > 0) {
        ...
        for (const neighbor of neighbors) {
            ...
            const hScore = Math.abs(neighbor.x - realEnd.x) + Math.abs(neighbor.y - realEnd.y); // Manhattan
            const newNode = {
                x: neighbor.x,
                y: neighbor.y,
                f: tentativeG + hScore,
