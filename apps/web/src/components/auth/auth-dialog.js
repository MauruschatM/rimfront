...
export function AuthDialog({ children, defaultTab = "login", }) {
    ...
    return (<Dialog onOpenChange={setOpen} open={open}>
      ...
      <DialogContent className="pixel-border bg-card text-card-foreground sm:max-w-md">
        ...
        <Tabs className="w-full" onValueChange={(v) => setActiveTab(v)} value={activeTab}>
          <TabsList className="grid w-full grid-cols-2 bg-muted/50 p-1">
            <TabsTrigger className="font-mono text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground" value="login">
              LOGIN
            </TabsTrigger>
            <TabsTrigger className="font-mono text-xs data-[state=active]:bg-primary data-[state=active]:text-primary-foreground" value="signup">
              SIGN UP
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent className="space-y-4 pt-4" value="login">
            <Form {...loginForm}>
              <form className="space-y-4" onSubmit={loginForm.handleSubmit(onLogin)}>
                <FormField control={loginForm.control} name="email" render={({ field }) => (<FormItem>
                      <FormLabel className="font-mono text-xs uppercase">
                        Email
                      </FormLabel>
                      <FormControl>
                        <Input className="font-mono text-sm" placeholder="pilot@rimfront.net" {...field}/>
                      </FormControl>
                ...
                <FormField control={loginForm.control} name="password" render={({ field }) => (<FormItem>
                      ...
                      <FormControl>
                        <Input className="font-mono text-sm" type="password" {...field}/>
                      </FormControl>
                      <FormMessage className="font-mono text-xs"/>
                    </FormItem>)}/>
                <Button className="pixel-corners w-full rounded-none bg-primary font-sans text-primary-foreground hover:bg-primary/90" disabled={isLoading} type="submit">
          ...
          <TabsContent className="space-y-4 pt-4" value="signup">
            <Form {...signupForm}>
              <form className="space-y-4" onSubmit={signupForm.handleSubmit(onSignup)}>
                <FormField control={signupForm.control} name="name" render={({ field }) => (<FormItem>
                      <FormLabel className="font-mono text-xs uppercase">
                        Callsign (Name)
                      </FormLabel>
                      <FormControl>
                        <Input className="font-mono text-sm" placeholder="Skywalker" {...field}/>
                      </FormControl>
                      <FormMessage className="font-mono text-xs"/>
                    </FormItem>)}/>
