...
export default function Home() {
    ...
    return (<main className="relative flex h-full w-full flex-col items-center justify-center overflow-hidden">
      ...
      <div className="stars">
        ...
        */}
        {Array.from({ length: 50 }).map((_, i) => (<div className="star" key={i} style={{
                top: `${Math.random() * 100}%`,
      ...
      <div className="z-10 flex flex-col items-center gap-8">
        ...
        {session ? (<div className="flex flex-col items-center gap-8">
            {/* Top Center Play Button */}
            <ModeSelector user={session.user}/>
            <UserProfile user={session.user}/>
          </div>) : (<div className="fade-in slide-in-from-bottom-8 flex animate-in flex-col gap-4 duration-1000">
            <AuthDialog defaultTab="login">
              <Button className="pixel-corners pixel-border h-16 rounded-none border-2 border-primary bg-background px-12 font-sans text-primary text-xl transition-all hover:bg-primary hover:text-primary-foreground" size="lg">
                START GAME
              </Button>
            </AuthDialog>

            <div className="flex justify-center gap-4">
              <Button className="font-mono text-muted-foreground text-xs" variant="link">
