  ...
  if (member.path && member.path.length > 0) {
    ...
    if (target && (member.x !== target.x || member.y !== target.y)) {
      ...
      if (!member.stateEnd || now > member.stateEnd) {
        ...
        member.stateEnd = now + (Math.random() * 10_000 + 5000);
      }
    }
  }
  return dirty;
...
export const endPlacementPhase = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (game.phase === "placement") {
      ...
      // Start both loops
      await ctx.scheduler.runAfter(0, internal.game.round, {
        gameId: game._id,
      });
      await ctx.scheduler.runAfter(0, internal.game.game_tick, {
        gameId: game._id,
      });
    }
  },
  ...
  handler: async (ctx, args) => {
    ...
    for (const chunk of unitChunks) {
      ...
      const processMember = (
        ...
        if (searchRange > 0) {
          const enemy = spatial.findClosestTarget(
            member.x,
            member.y,
            searchRange,
            member.ownerId
          ...
          if (enemy) {
            ...
            if (enemy.dist <= requiredRange) {
              ...
              if (enemy.type === "unit") {
                ...
                const realBuilding = mapDoc.buildings.find(
                  (mb) => mb.id === enemy.target.id
                );
                if (realBuilding && realBuilding.ownerId !== member.ownerId) {
                  realBuilding.captureProgress =
                    (realBuilding.captureProgress || 0) + GAME_TICK_RATE / 1000;
      ...
      for (const troop of chunk.troops) {
        ...
        troop.soldiers = troop.soldiers.filter(
          (s: any) => !deadUnitIds.has(s.id)
        );
    ...
    if (mapDirty) {
      ...
      if (owners.size === 1 && bases.length > 0) {
        ...
        // Schedule deletion
        await ctx.scheduler.runAfter(10_000, api.game.deleteGame, {
          gameId: args.gameId,
        });
        return; // Stop loop
      }
    ...
    await ctx.scheduler.runAfter(GAME_TICK_RATE, internal.game.game_tick, {
      gameId: args.gameId,
    });
  },
});
...
export const round = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const chunk of unitChunks) {
      ...
      for (const family of chunk.families) {
        ...
        for (const member of family.members) {
          // Only process economy/state timeouts, NOT movement (handled in game_tick)
          if (member.state === "working" && member.ownerId) {
            playerCredits[member.ownerId] =
              (playerCredits[member.ownerId] || 0) + 1000;
          }
        }
      }
      ...
      for (const troop of chunk.troops) {
        ...
        if (troop.soldiers.length < 10) {
          // Max 10 soldiers
          const lastSpawn = troop.lastSpawnTime || 0;
          // Random 10-30s -> 10000 - 30000ms
          const nextSpawnDelay = 10_000 + Math.random() * 20_000;
    ...
    for (const chunk of unitChunks) {
      let dirty = false;
      for (const fam of chunk.families) {
        if (fam.members.length < 4 && Math.random() < 0.2) {
          // Chance to spawn
          // Find home location
    ...
    await ctx.scheduler.runAfter(5000, internal.game.round, {
      gameId: args.gameId,
    });
  },
});
...
export const moveTroop = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (foundTroop.commander.ownerId !== player._id)
