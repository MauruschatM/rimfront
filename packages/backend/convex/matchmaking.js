...
export const findOrCreateLobby = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const waitingGames = await ctx.db
            .query("games")
            .filter((q) => q.and(q.eq(q.field("status"), "waiting"), q.eq(q.field("type"), args.type), q.eq(q.field("subMode"), args.subMode)))
            .take(5);
            ...
            if (slotsNeeded > 0) {
                if (game.subMode !== "ffa") {
                    ...
                    for (const team of teams) {
                        while ((teamCounts[team._id] || 0) < MAX_PER_TEAM &&
                    ...
                    while (botsCreated < slotsNeeded) {
                        ...
                        if (targetTeamId) {
                            ...
                            while ((teamCounts[targetTeamId] || 0) < MAX_PER_TEAM &&
                                botsCreated < slotsNeeded) {
                                await ctx.db.insert("players", {
                                    gameId: game._id,
                                    isBot: true,
