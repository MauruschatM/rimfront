...
const MapRenderer = React.memo(function MapRenderer({ map }: { map: any }) {
  ...
  const palette = React.useMemo(
    () => createPlanetPalette(planetType),
    [planetType]
  );
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (tileId !== 0) {
          ...
          switch (tileId) {
            case 1:
              color.set(palette.dirt);
              break;
            case 2:
              color.set(palette.sand);
              break;
            case 3:
              color.set(palette.sand);
              break; // Grass -> fallback
            case 6:
              color.set(palette.rock);
              break;
            case 7:
              color.set("#ff3300");
              break; // Lava
  ...
  return (
    <instancedMesh args={[undefined, undefined, width * height]} ref={meshRef}>
      <planeGeometry args={[TILE_SIZE, TILE_SIZE]} />
      <meshBasicMaterial color="white" map={noiseTexture} />
    </instancedMesh>
  );
});
  ...
  return (
    <group>
      {buildings.map((b: any, i: number) => {
        ...
        const capturePct = b.captureProgress
          ? Math.min(b.captureProgress / 5, 1)
          : 0;
...
function UnitsRenderer({
  ...
  React.useEffect(() => {
    ...
    if (familiesRef.current.instanceColor)
      familiesRef.current.instanceColor.needsUpdate = true;
  }, [families]);
  ...
  React.useEffect(() => {
    ...
    if (commandersRef.current.instanceColor)
      commandersRef.current.instanceColor.needsUpdate = true;
  ...
  React.useEffect(() => {
    ...
    if (soldiersRef.current.instanceColor)
      soldiersRef.current.instanceColor.needsUpdate = true;
  ...
  return (
    <group>
      {/* Lasers */}
      {lasers.map((l, i) => (
        <Line color={l.color} key={i} lineWidth={1} points={[l.start, l.end]} />
      ))}
  ...
  if (game.status === "ended") {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80">
        <div className="fade-in zoom-in animate-in text-center duration-1000">
          <h1 className="mb-4 font-bold font-pixel text-6xl text-yellow-400">
            GAME OVER
          </h1>
